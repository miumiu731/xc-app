 ...
 * @param round 1=四舍五入后格式化；0=向下取整 默认四舍五入
 * @returns {string}
 */
export function kpiFormatter(num, pattern, round = 1) {
  if (!pattern || num == null) {
    return num || '-'
  } else {
    if (pattern.indexOf('%') > 0) {
      ...
      const result = formatNumber(temp, pattern.substr(0, pattern.indexOf('%')), round)
      return result + '%'
    } else {
      return formatNumber(num, pattern, round)
    }
  }
}
